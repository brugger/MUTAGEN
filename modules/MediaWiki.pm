...
#
# Changed by Kim Brugger so it now is possible to download the html formatted
# entry page and not the edit page. The code was furthermore integrated into 
# MUTAGEN to create a link between it and the SulfolobusWiki.
# Kim Br�gger ( kbr@brugger.dk )
#
...
sub getHtmlPage {
	...
	Debug "$args{'protocol'}://$WHOST/$WPATH/index.php?title=$args{'title'}" if $self->{'debug'};
	my @lines = split /\n/, $resp->content();
	...
	for (my $jj = 0; $jj <= $#lines; $jj++) {
		...
		} elsif ($line =~ m/<div class="printfooter">/) {
		  push (@content, $line) if ($saving && $1);
		  $saving = 0;
		  last;
		}

		next if ($line =~ /jump-to-nav/);
		push (@content, $line) if ($saving);
	}
